import { Calendar, Users, CheckCircle, TrendingUp, Heart } from 'lucide-react';
...
export default function PackageCard({ pkg, onBook }: PackageCardProps) {
  ...
  const handleHeartClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (!user) return;
    toggleWishlist('package', pkg.id);
  };

  return (
    <div className="group bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-gray-100 hover:-translate-y-1 flex flex-col animate-fade-in">
      <div className="relative overflow-hidden h-52">
        ...
        {user && (
          <button onClick={handleHeartClick} className="absolute top-3 right-3 w-8 h-8 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center hover:scale-110 transition-transform">
